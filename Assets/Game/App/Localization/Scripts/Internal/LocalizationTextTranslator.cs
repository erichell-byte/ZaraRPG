...
{
    public sealed class LocalizationTextTranslator : ITranslator<string>
    {
        private readonly Dictionary<string, ITranslator<string>> translators;

        private readonly string[] pageSeparator;

        public LocalizationTextTranslator(LocalizationTextConfig config)
        {
            ...
            {
                var page = pages[i];
                this.translators[page.name] = new TextTranslator(page.entities);
            }
